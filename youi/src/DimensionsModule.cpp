...
//    return;
    ...
    if (pScene)
    {
        ...
        const auto surfaceSize = glm::vec2{static_cast<float>(width), static_cast<float>(height)};

        const auto screenSize = glm::vec2{static_cast<float>(width > height ? 1920 : 1080), static_cast<float>(width > height ? 1080 : 1920)};
        auto scale = surfaceSize / screenSize;

        pReactNativeView->SetSize(glm::vec3{screenSize, 1.f});
        pReactNativeView->SetScale(glm::vec3{scale, 1.f});
    }
}
